...
typedef struct tuple_graph {
  Ring::GlobalAddress<packed_edge> edges;
  int64_t nedge; /* Number of edges in graph, in both cases */
} tuple_graph;

struct csr_graph {
  Ring::GlobalAddress<int64_t> xoff, xadj, xadjstore;
  int64_t nv;
  int64_t nadj;
};
...
double make_bfs_tree(csr_graph * g, Ring::GlobalAddress<int64_t> bfs_tree, int64_t root);
